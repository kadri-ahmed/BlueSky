...
function createWorker(self) {
    ...
    function processPlyBuffer(inputBuffer) {
        ...
        const labelBuffer = new Uint16Array(vertexCount);
        ...
        for (let j = 0; j < vertexCount; j++) {
            ...
            if (types["label"]) {
                const label = dataView.getUint16(
                    j * 18 + offsets.label,
                    true
                );
                labelBuffer[j] = label;
                if(j < 20) {
                    console.log(labelBuffer[j]); // REMOVE
                }
            }
        }
        console.timeEnd("build buffer");
        return { buffer, labelBuffer };
    }
    ...
    self.onmessage = (e) => {
        if (e.data.ply) {
            console.log("Processing PLY");
            vertexCount = 0;
            runSort(viewProj);
            buffers = processPlyBuffer(e.data.ply);
            buffer = buffers.buffer;
            labelBuffer = buffers.labelBuffer;
            vertexCount = Math.floor(buffers.buffer.byteLength / rowLength);
            postMessage({ buffer: buffers.buffer, save: !!e.data.save });
        } else if (e.data.buffer) {
            console.log("Updating Buffer and Vertex Count");
            buffer = e.data.buffer;
            vertexCount = e.data.vertexCount;
        } else if (e.data.vertexCount) {
            console.log("Updating Vertex Count");
            vertexCount = e.data.vertexCount;
        } else if (e.data.view) {
            console.log("Updating View");
            viewProj = e.data.view;
            throttledSort();
        } else if (e.data.similarityScores) {
            handleSimilarityScores(e.data.similarityScores);
        }
    };

    function handleSimilarityScores(scores) {
        console.log("Handling similarity scores:", scores);
        
        // Find the min and max similarity scores for normalization
        const similarities = Object.values(scores);
        const minSim = Math.min(...similarities);
        const maxSim = Math.max(...similarities);
        
        // Skip if no valid range
        if (maxSim === minSim) return;

        // Function to convert similarity score to RGB color
        // Blue (0,0,255) for highest similarity
        // Red (255,0,0) for lowest similarity
        function similarityToColor(similarity) {
            // Normalize similarity to 0-1 range
            const normalized = (similarity - minSim) / (maxSim - minSim);
            
            // Create a more distinct color transition:
            // Low similarity (0): bright red (255, 0, 0)
            // Mid similarity (0.5): bright green (0, 255, 0)
            // High similarity (1): bright blue (0, 0, 255)
            let r, g, b;
            ...
            return [r, g, b, 255]; // Full alpha
        }

        if (!buffer || !labelBuffer){
            console.log("No labels for this model. Query is not supported.");
            return;
        } 

        // Update the colors in the buffer based on labels and similarity scores
        const vertexCount = Math.floor(buffer.byteLength / rowLength);
        for (let i = 0; i < vertexCount; i++) {
            const label = labelBuffer[i];
            if (label !== undefined && scores[label] !== undefined) {
                const color = similarityToColor(scores[label]);
                
                // Update the color in the buffer
                // Colors are stored at offset 24 in each vertex row (after position and scale)
                const colorOffset = i * rowLength + 24;
                const colorView = new Uint8Array(buffer, colorOffset, 4);
                colorView.set(color);
            }
        }

        // Signal that the texture needs to be regenerated
        console.log("Regenerating Texture");
        generateTexture();
    }
}
...
async function main() {
    ...
    window.addEventListener("keydown", (e) => {
        ...
        if (e.target.tagName === 'INPUT') {
            return; // Skip handling keyboard events when input is focused
        }
    ...
    window.handleQueryResults = (similarityScores) => {
        if (worker) {
            worker.postMessage({ similarityScores });
        }
    };
}
